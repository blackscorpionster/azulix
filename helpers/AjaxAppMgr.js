// JavaScript Document
function callAjaxApp(forma, type, urlDestino)
{
	/*if(accion)
	...
	*/
	...
	var request = YAHOO.util.Connect.asyncRequest(type, urlDestino, callback);
...
function YUIErrorManagger(mensajes, errores)
{
	...
	var notes = APP.getinfo("notes");
	var problems = APP.getinfo("problems");
...
{
	//alert(datos);
	var divTmp = APP.tmpFormContainer.cust_obj;//document.getElementById("div_main_form");
	var cantDatos = datos.length;
	for(i=0;i<cantDatos;i++)
	{
		arrDatos = datos[i];
